...
export async function getBadges(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const stats = await withConn(async (conn) => {
      const messages = await conn.execute(`SELECT COUNT(*) AS CNT FROM MESSAGES WHERE ATTENDEE_ID = :id`, { id: attendeeId }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
      const checkins = await conn.execute(`SELECT COUNT(*) AS CNT FROM CHECKINS c JOIN REGISTRATIONS r ON r.ID = c.REGISTRATION_ID WHERE r.ATTENDEE_ID = :id`, { id: attendeeId }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
      // No direct schema for speaker; naive heuristic: sessions where speaker name matches attendee name
      const attendee = await conn.execute(`SELECT NAME FROM ATTENDEES WHERE ID = :id`, { id: attendeeId }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
      const name = String((attendee.rows as Array<{NAME: string}>)[0]?.NAME || '');
      const sessions = await conn.execute(`SELECT COUNT(*) AS CNT FROM SESSIONS WHERE SPEAKER = :name`, { name }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
      return {
        messages: Number((messages.rows as Array<{CNT: number}>)[0]?.CNT || 0),
        checkins: Number((checkins.rows as Array<{CNT: number}>)[0]?.CNT || 0),
        sessionsSpoken: Number((sessions.rows as Array<{CNT: number}>)[0]?.CNT || 0)
      };
    });
