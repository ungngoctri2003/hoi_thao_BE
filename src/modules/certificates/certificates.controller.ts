...
export async function generateCertificate(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const data = await withConn(async (conn) => {
      const attendee = await conn.execute(`SELECT ID, NAME, EMAIL FROM ATTENDEES WHERE ID = :id`, { id: attendeeId }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
      const conference = await conn.execute(`SELECT ID, NAME, START_DATE, END_DATE FROM CONFERENCES WHERE ID = :id`, { id: conferenceId }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
