...
import { authenticateSocket, AuthenticatedSocket } from '../middlewares/socket-auth';
...
export function initSocket(server: HttpServer) {
  ...
  try {
    ...
    if (messagesNs) {
      messagesNs.use(authenticateSocket);

      messagesNs.on('connection', (socket: AuthenticatedSocket) => {
        logger.info(
          {
            socketId: socket.id,
          ...
          'User connected to main namespace'
        );

        // Setup messaging socket handlers
        setupMessagingSocket(socket);

        socket.on('join', (room: string) => {
          socket.join(room);
          logger.info({ socketId: socket.id, room, userId: socket.userId }, 'User joined room');
        });

        socket.on('leave', (room: string) => {
          socket.leave(room);
          logger.info({ socketId: socket.id, room, userId: socket.userId }, 'User left room');
        });

        socket.on('disconnect', () => {
          logger.info(
            { socketId: socket.id, userId: socket.userId },
            'User disconnected from main namespace'
          );
        });
      });
    }
...
export function emitUserPermissionChange(
  userId: number,
  payload: {
...
export function emitNotificationStats(
  userId: number,
  stats: {
...
export function emitBulkNotificationUpdate(
  userIds: number[],
  payload: {
