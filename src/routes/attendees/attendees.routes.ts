...
import {
  ...
  listByConference,
  listWithConferences,
} from '../../modules/attendees/attendees.controller';

export const attendeesRouter = Router();

attendeesRouter.get('/', auth(), rbac('attendees.read'), list);
attendeesRouter.get('/with-conferences', auth(), rbac('attendees.read'), listWithConferences);
attendeesRouter.post(
  '/',
  auth(),
  rbac('attendees.write'),
  audit('data', 'create', 'attendee'),
  create
);
...
attendeesRouter.post(
  ...
  auth(),
  rbac('attendees.write'),
  audit('conference', 'create', 'registration'),
  createRegistration
);
